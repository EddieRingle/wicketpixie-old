			...
			<div class="wrap">
				...
				<div id="admin-options">
					...
					?>		
						<tr>
							<td><?php echo $service->title; ?></td>
						   	<td style="text-align:center;"><?php echo $service->feed_url; ?></td>
						   	<td style="text-align:center;">
							<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=notify.php&amp;gather=true&amp;id=<?php echo $service->id; ?>">
					...
					<?php if ( isset( $_REQUEST['gather'] ) ) { ?>
						<?php $data= $notify->gather( $_REQUEST['id'] ); ?>
						<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=notify.php&amp;edit=true">
					...
					<?php } ?>
					<?php if( $notify->check() != 'false' && !isset( $_REQUEST['gather'] ) ) { ?>
						<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=notify.php&amp;add=true" class="form-table">
							<h2>Add a Service</h2>
							<p><select name="service" id="title" onblur="var service = this.value">
                            ...
                            <!--
                                if(service != "ping.fm")
                                {
                                    document.write("<?php echo "<p><input type=\"text\" name=\"username\" id=\"url\" onfocus=\"if(this.value=='Username')value=''\" onblur=\"if(this.value=='')value='Username';\" value=\"Username\" /></p>\n<p><input type=\"text\" name=\"password\" id=\"url\" onfocus=\"if(this.value=='Password')value=''\" onblur=\"if(this.value=='')value='Password';\" value=\"Password\" /></p>"; ?>";
                                }
                                
                                if(service == "ping.fm")
                                {
                                    document.write("<?php echo "<p><input type=\"text\" name=\"apikey\" id=\"url\" onfocus=\"if(this.value=='API Key')value=''\" onblur=\"if(this.value=='')value='API Key';\" value=\"API Key\" /></p>"; ?>";
                                }
                            -->
                            </script>
                                <p class="submit">
...
add_action ('admin_menu', array( 'NotifyAdmin', 'addFavesMenu' ) );
NotifyAdmin::install();
?>
